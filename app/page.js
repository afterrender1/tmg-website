...
export default function HomePage() {
  ...
  // Detect screen width
  useEffect(() => {
    const checkWidth = () => setIsDesktop(window.innerWidth >= 768); // md breakpoint
    checkWidth();
      ...
      <div className="absolute inset-0 flex flex-col justify-center items-center px-6 md:px-12">
        ...
        <div className="relative max-w-7xl w-full flex flex-col md:flex-row items-center justify-between">
          ...
          <div
            ...
            onMouseEnter={() => isDesktop && setHoverSide("left")}
            onMouseLeave={() => isDesktop && setHoverSide(null)}
            onClick={() => router.push("/buy")}
          >
            <motion.div
              animate={{ x: isDesktop && hoverSide === "left" ? -15 : 0 }}
              transition={{ type: "spring", stiffness: 200 }}
              className="flex items-center gap-6"
            >
              <Home size={40} className="text-white" />
              <h2 className={`${novaFlat.className} text-3xl md:text-6xl font-semibold text-white tracking-tight`}>
                SHOWROOM
              </h2>
            </motion.div>
          ...
          <div
            ...
            onMouseEnter={() => isDesktop && setHoverSide("right")}
            onMouseLeave={() => isDesktop && setHoverSide(null)}
            onClick={() => router.push("/customisation")}
          >
            <motion.div
              animate={{ x: isDesktop && hoverSide === "right" ? 15 : 0 }}
              transition={{ type: "spring", stiffness: 200 }}
        ...
        {isDesktop && (
          <AnimatePresence>
            {hoverSide && (
              ...
              >
                {hoverSide === "left" && (
                  <div className="text-left max-w-md text-white space-y-2" style={{ fontFamily: montserrat.style.fontFamily }}>
                ...
                {hoverSide === "right" && (
                  <div className="text-left max-w-md ml-auto text-white space-y-2" style={{ fontFamily: montserrat.style.fontFamily }}>
